...
def make_dataset_val(root, source):
    anno_path = source
    anno = json.load(open(anno_path, 'r'))
    clips = []
    for ann in anno:
        clip_path = ann['path']
        duration = ann['num_frames'] - 1
        target = ann['label']
        if ann['value'] > 0.4:
            pred_text = ann['pred']
        else:
            pred_text = ''
        item = (clip_path, duration, target, pred_text)
        clips.append(item)
    return clips
...
def get_all_text_feat():
    ...
    for i in range(11):
        ...
        all_text_feat.append(np.load(path))
    return all_text_feat
        

class ARID_MOD(data.Dataset):
    ...
    def __init__(self,
        ...
        if phase == "train":
            ...
            clips = make_dataset_val(root, source)
        
        self.all_text_feat = get_all_text_feat()
        self.gamma = gamma
        # clips:(视频名称, 帧长, 标签)
