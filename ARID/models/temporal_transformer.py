...
class visual_prompt(nn.Module):
    def __init__(self, sim_head, clip_state_dict, T):
        ...
        self.T = T
        assert sim_head in ["meanP", "LSTM", "Transf", "Conv_1D", "Transf_cls", 'joint']
        ...
        if self.sim_header == "LSTM" or self.sim_header == "Transf" or self.sim_header == "Transf_cls" or self.sim_header == "Conv_1D":
            ...
            # print(embed_dim, context_length, vocab_size, transformer_width, transformer_heads)  512 77 49408 512 8
        if self.sim_header == 'joint':
            self.joint_num_patches = 8 * ((224 // 32) * (224 // 32))
            self.joint_pos_embed = get_sinusoid_encoding_table(self.joint_num_patches, 768)

            self.joint_transformer = Transformer(768, 2, 768, dropout=None)
            self.norm = LayerNorm(768)
            scale = 768 ** -0.5
            self.proj = nn.Parameter(scale * torch.randn(768, 512))
        if self.sim_header == "adapter":
            embed_dim = clip_state_dict["text_projection"].shape[1]
    ...
    def forward(self, x, x_original=None):
        ...
        if self.sim_header == "meanP":
            ...
            x = x.type(x_original.dtype) + x_original
        elif self.sim_header == "joint":

            B = x.shape[0] // self.T
            x = x[:, 1:, :]
            x = rearrange(x, '(b t) n m -> (b n) t m', b=B, t=self.T)
            x = rearrange(x, '(b n) t m -> b (n t) m', b=B, t=self.T)
            x = x + self.joint_pos_embed.expand(B, -1, -1).type_as(x).to(x.device).clone().detach()
            ...
            if self.proj is not None:
                x = x @ self.proj
            return x[:, 0].type(x_original.dtype) + x_original
