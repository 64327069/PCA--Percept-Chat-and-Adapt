...
class KnowledgeAdapterBlock(nn.Module):
    ...
    def forward(self, visual_feat):
        ...
        # self attention 第一层要不要就待定
        visual_feat = rearrange(visual_feat, ' n (b t) d -> b t n d', t = 8)
        
        # visual_feat = rearrange(visual_feat, 'b n t d -> b t n d')
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x, clip_feat = None, text_feat = None, T = 8):
        if clip_feat == None and text_feat == None:
            ...
            x =  x + 0.0 * self.cross_attn(clip_feat, x) + 0.02 * self.cross_attn_text(text_feat, x)
            x = rearrange(x, 'b t n d ->  n (b t) d')
        else:
...
class Transformer(nn.Module):
    ...
    def forward(self, x, clip_feat, text_feat = None):
        for i in range(len(self.resblocks) - 1):
            x = self.resblocks[i](x)
        x = self.resblocks[-1](x, clip_feat, text_feat)
        return x
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_feat = None, text_feat = None):
        ...
        x_feat = self.transformer(x, x_feat, text_feat)
        # 197 64 768
        x = x_feat.permute(1, 0, 2)  # LND -> NLD
        # add ADAKA Here
        #  64 197 768
...
if __name__ == '__main__':
    ...
    model, clip_state_dict = clip_mod.load_mod(config.network.arch,device='cuda',jit=False, tsm=config.network.tsm, T=config.data.num_segments,dropout=config.network.drop_out, emb_dropout=config.network.emb_dropout,pretrain=config.network.init, joint = config.network.joint) 
    model = model.visual
    model.float()
    video = torch.randn(64, 3, 224, 224).cuda().float()
    video_feat = torch.randn(1576, 8, 768).cuda().float()
    text_feat = torch.randn(8, 160, 768).cuda().float()
    output, output_feat = model(video, video_feat, text_feat)
