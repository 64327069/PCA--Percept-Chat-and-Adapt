...
if __name__ == '__main__':
    ...
    model, clip_state_dict = clip.load(config.network.arch,device='cuda',jit=False, tsm=config.network.tsm, T=config.data.num_segments,dropout=config.network.drop_out, emb_dropout=config.network.emb_dropout,pretrain=config.network.init, joint = config.network.joint) 
    model = model.visual
    video = torch.randn(64, 3, 224, 224)
    output = model(video)
