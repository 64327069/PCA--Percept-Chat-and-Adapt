...
if __name__ == '__main__':
    ...
    import yaml
    config = '/opt/data/private/workplace/pipe2/exp/clip/ViT-B-16.pt/Tube/20230805_155525/tube_train.yaml'
    with open(config, 'r') as f:
        config = yaml.full_load(f)
    config = DotMap(config)

    model, clip_state_dict = clip_extract.load(config.network.arch,device='cuda',jit=False, tsm=config.network.tsm, T=config.data.num_segments,dropout=config.network.drop_out, emb_dropout=config.network.emb_dropout,pretrain=config.network.init, joint = config.network.joint) 
    model = model.visual
    model.float()
