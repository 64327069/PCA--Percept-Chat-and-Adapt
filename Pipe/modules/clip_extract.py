...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        all_feat = x.unsqueeze(0)
        # import ipdb; ipdb.set_trace()
        for blk in self.resblocks[1:]:
            x = blk(x)
            # append tensor to a tensor list
            feat = x.unsqueeze(0)
            all_feat = torch.cat([all_feat, feat], dim = 0)
        return x, all_feat[-1]

# generate a tensor whose dim is [8, 197, 768]
test_tensor = torch.randn(8, 197, 768)
...
@register_model
def clip_feat_b16(
    pretrained_path = None, input_resolution=224, kernel_size=1,
...
if __name__ == '__main__':
    ...
    model = clip_feat_b16(pretrained_path = None)
    video = torch.randn(8, 3, 8, 224, 224)
    out, all_feat = model(video)
    
    print(out.shape, all_feat.shape)
