...
class visual_prompt(nn.Module):
    def __init__(self, sim_head, embed_dim, T, num_layer = 6):
        super().__init__()
        self.sim_header = sim_head
        self.T = T
        assert sim_head in ["meanP", "LSTM", "Transf", "Conv_1D", "Transf_cls"]
        self.frame_position_embeddings = nn.Embedding(77, embed_dim)
        if self.sim_header == "Conv_1D" :
            raise NotImplementedError("Not implemented yet")

        if self.sim_header == "Transf" :
            self.transformer = TemporalTransformer(width=embed_dim, layers=num_layer, heads=8)
        ...
        if self.sim_header == "Transf_cls":
            # n_layer has changed
            self.transformer = TAggregate(clip_length=self.T, embed_dim=embed_dim, n_layers=1)
    ...
    def forward(self, x):
        ...
        if self.sim_header == "meanP":
            ...
            x = x.type(x_original.dtype) + x_original
            return x.mean(dim=1, keepdim=False), x_feat
